...
class MotionDataset(Dataset):
    ...
    def __init__(self, pkl_path, rot_req, offset_frame, past_frame, future_frame, dtype=np.float32, limited_num=-1):
        ...
        self.inbetween_size_list = np.arange(10, future_frame, 10)
    ...
    def __getitem__(self, idx):
        ...
        inbetween_size = int(self.inbetween_size_list[np.random.randint(0, self.inbetween_size_list.size)])
        end_frame_idx = inbetween_size
        ...
        mask[0:tta.size] = tta
        tta = mask.copy()
        self.mask = np.array( [mask_itm > 0 for mask_itm in mask] )
        ...
        return {
            ...
            'conditions': {
                ...
                'style_idx': style_idx,
                'mask': self.mask,
                'keyframe_start': start_frame,
                'keyframe_end': end_frame,
                'inbetween_size': inbetween_size,
                'tta': tta,
            }
        }     
...
if __name__ == '__main__':
    ...
    rot_req = '6d'
    train_data = MotionDataset(pkl_path, rot_req, offset_frame=1, past_frame=10, future_frame=55, dtype=np.float32)
    train_dataloader = torch.utils.data.DataLoader(train_data, batch_size=16, shuffle=True, num_workers=8, drop_last=False, pin_memory=True)
    ...
    if do_export_test:
        ...
        bs_sequence = data['data'].permute(0, 2, 3, 1)
        cond = data['conditions']
        save_dir = 'save/visualization'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        for seq_idx, sequence in enumerate(bs_sequence):
            seq_file_name = 'sequence_' + str(seq_idx) + '_1x55x26x6.txt'
            seq_file_name = os.path.join(save_dir, seq_file_name)
            sequence = sequence.numpy()
            sequence.tofile(seq_file_name, sep='\n')
            print('saving sequence %s', seq_file_name)
